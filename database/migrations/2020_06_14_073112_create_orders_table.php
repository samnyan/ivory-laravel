...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->integer('product_count')->nullable(false)->comment('不同产品种类的数量');
            $table->float('product_amount_total')->nullable(false)->comment('商品总价');
            $table->float('order_amount_total')->comment('实付款金额，默认等于商品总价');
            $table->float('logistics_fee')->nullable(false)->comment('运费');
